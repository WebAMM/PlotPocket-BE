...
const forYou = async (req, res) => {
  try {
    ...
    if (uniqueCategoriesArray.length === 1) {
      ...
      const latestSeries = await Series.find({ category: latestCategory })
        .select("_id")
      const middleSeries = await Series.find({ category: middleCategory })
        .select("_id")
      const oldestSeries = await Series.find({ category: oldestCategory })
        .select("_id")
